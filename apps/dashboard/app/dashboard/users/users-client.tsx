...
interface Login { id: string; created_at: string; phone_number: string | null }
interface Scan {
  id: string;
  scanned_at: string;
  result: string;
...
interface UserActivity { logins: Login[]; scans: Scan[] }
...
function ActivityModal({ userId, userName, onClose }: { userId: string; userName: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[80vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="overflow-y-auto flex-1 p-6 space-y-6">
          {isLoading ? (
            ...
            <>
              <section>
                <h4 className="text-sm font-semibold text-gray-700 mb-3">Login History <span className="font-normal text-gray-400">({data?.logins.length ?? 0})</span></h4>
                {data?.logins.length ? (
              ...
              <section>
                <h4 className="text-sm font-semibold text-gray-700 mb-3">Scan History <span className="font-normal text-gray-400">({data?.scans.length ?? 0})</span></h4>
                {data?.scans.length ? (
                  <div className="border border-gray-200 rounded-lg overflow-hidden">
                    <table className="w-full text-sm">
                      <thead className="bg-gray-50 border-b border-gray-200">
                      ...
                      <tbody>
                        {data.scans.map((s, i) => (
                          <tr key={s.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                            ...
                            <td className="px-4 py-2"><Badge variant={s.result === 'granted' ? 'success' : 'error'}>{s.result}</Badge></td>
...
export default function UsersClient({ initialUsers }: { initialUsers: User[] }) {
  ...
  async function handleRoleChange(userId: string, roleType: string) {
    const prev = users.find(u => u.id === userId)?.role_type;
    setUpdating(userId);
    setUsers(all => all.map(u => u.id === userId ? { ...u, role_type: roleType } : u));
    try {
      await api.patch(`/admin/users/${userId}/role`, { role_type: roleType });
      toast.success('Role updated');
    } catch (e: unknown) {
      if (prev) setUsers(all => all.map(u => u.id === userId ? { ...u, role_type: prev } : u));
      ...
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            {filtered.length} {search ? 'found' : `user${filtered.length !== 1 ? 's' : ''} on the platform`}
          </p>
        </div>
            ...
            {filtered.length === 0 ? (
              ...
              <>
                <TableRow key={user.id}>
                  <TableCell className="font-mono text-xs text-gray-700">{user.phone_number ?? user.email ?? '—'}</TableCell>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <button onClick={() => setActivityUserId(user.id)}
                        ...
                        <tbody>
                          {user.memberships.map(m => (
                            <tr key={m.id} className="text-gray-600">
                              ...
                              <td className="py-1"><Badge variant={m.approval_status === 'approved' ? 'success' : 'muted'}>{m.approval_status}</Badge></td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
