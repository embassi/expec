...
export default function CommunitiesClient({ initialCommunities }: { initialCommunities: Community[] }) {
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState({ name: '', slug: '', type: '' });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');

  const { data: communities, isLoading } = useFetch<Community[]>('/admin/communities', { fallbackData: initialCommunities });
